...
//Low pass bessel filter order=4 alpha1=0.024 
// 24Hz filter for accelerometer
class  FilterBeLp4ACC
{
	public:
		FilterBeLp4ACC()
		{
	...
	public:
		float step(float x) //class II 
		{
			...
			v[2] = v[3];
			v[3] = v[4];
			v[4] = (1.206025189135900844e-4f * x)
				 + (-0.48985888026352442104f * v[0])
				 + (2.32021192307875612215f * v[1])
				 + (-4.14577774766348916557f * v[2])
				 + (3.31349506454564002311f * v[3]);
			return 
				 (v[0] + v[4])
				+4 * (v[1] + v[3])
				+6 * v[2];
...
FilterBeLp4ACC acclpf[3];

extern "C" float acchpf( float in, int num )
{
	return acclpf[num].step(in );
}
